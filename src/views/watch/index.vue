...
import type { Video } from '@/service/types/video'
import { onMounted, ref } from 'vue'
import { getUploadAuth } from '@/service/vod'
import type { ResVideoAuth } from '@/service/types/vod'
...
const createPlayer = (data:ResVideoAuth) => {
  ...
  //   eslint-disable-next-line
   new window.Aliplayer({
    ...
    width: '100%',
    height: '500px',
    vid: video.value?.vodVideoId, // 必选参数。音视频ID。示例：1e067a2831b641db90d570b6480f****。
    playauth: data.PlayAuth// 必选参数。音视频播放凭证。
  }, function (player:any) {
    onReady(player)
  })
  return p
...
onMounted(async () => {
  ...
  video.value = (res as any).video
  const vodVideoId = video.value?.vodVideoId
  if (vodVideoId) {
    const authData = await getUploadAuth(vodVideoId)
    const player = await createPlayer(authData)
    console.log(player)
  }
})
